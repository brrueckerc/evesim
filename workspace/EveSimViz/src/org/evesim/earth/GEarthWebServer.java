...
public class GEarthWebServer
{
	/**
	 * Stuff that has to do with the Jetty webserver
	 */
    
    final static Logger logger = Logger.getLogger(GEarthWebServer.class.getName());
	...
	*/
	 ...
	 */
	//public static GEarthWebServer instance = new GEarthWebServer();
	
	public GEarthWebServer(int port)
	{
		// copied from @author reder
        mServer = new Server(port);
        Context root = new Context(mServer, "/", Context.SESSIONS);
        root.addServlet(new ServletHolder(new GEarthStaticContentServlet()), "/default/*");
        root.addServlet(new ServletHolder(new GEarthKMLServlet()), "/kml/*");
        root.addServlet(new ServletHolder(new GEarthVelocityContentServlet()), "/vml/*");
	}
